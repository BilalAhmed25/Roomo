...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(200)
      .json({ user: userData, message: "Login successful.", token: token });
  } catch (error) {
...
router.post("/verify-account", authenticateToken, async (req, res) => {
  try {
    ...
    if (req.user && req.user.Email !== email) {
      return res
        .status(403)
        .json({ message: "You can only verify your own account." });
    }
...
router.post("/verify-otp", async (req, res) => {
  try {
    ...
    await con.execute("UPDATE UserDetails SET IsVerified = 1 WHERE Email = ?", [
      email,
    ]);
