...
router.post(
  ...
  async (req, res) => {
    const { aboutMe, ageRange, moveinTimeline, lifestylePreferences } =
      req.body;
    ...
    if (!aboutMe || !ageRange || !moveinTimeline || !lifestylePreferences) {
      return res.status(400).json({
        message:
          "All profile fields are required (aboutMe, ageRange, moveinTimeline, lifestylePreferences).",
      });
    }
    ...
    try {
      ...
      await con.execute(updateProfileQuery, [
        profilePictureUrl,
        aboutMe,
        ageRange,
        moveinTimeline,
